...
<p align="center">
  <a href="https://discord.gg/QRsPcNrSB" target="_blank" rel="noopener noreferrer">
    <img src="assets/discord_button_ja.svg" alt="Discord コミュニティに参加" height="38" />
  </a>
  &nbsp;&nbsp;
  <a href="https://antigravity-enhance-tools.github.io/" target="_blank" rel="noopener noreferrer">
    <img src="assets/website_button_ja.svg" alt="公式サイトへアクセス" height="38" />
  </a>
</p>
...
<p align="center">
  <a href="https://antigravity-enhance-tools.github.io/">🌐 公式サイト</a> • 
  <a href="#-機能一覧">機能一覧</a> • 
...
チャットツールバーからワンクリックで展開し、35種類以上の設計・開発スキルを検索・呼び出し。新カテゴリ「📦 その他」やネイティブ/カスタム識別バッジ、GitHub ワンクリック追加に対応：

| 視覚的スキルハブ (35種類以上の厳選スキル、所属バッジ、GitHub ワンクリック追加) |
| :---: |
| <img src="assets/verified_skills_hub_live.png" width="750" /> |

### 5. サイドバープラグインエコシステムセンター (Plugin Center) 実機キャプチャ
左側サイドバーに「@ プラグイン」ハブをネイティブ配置。OpenAI Codex ローカル拡張機能と Antigravity プラグインを統合管理（対話欄ワンクリック呼び出し対応）：

| サイドバープラグインセンター (Documents / Presentations / Spreadsheets / PDF Tools など 9 種類の主要プラグイン) |
| :---: |
...
| マルチアカウント管理 & 即時切替モーダル | ステータスバー統合プロファイル & ツールチップ |
| :---: | :---: |
| <img src="assets/verified_account_switcher_live.png" width="450" /> | <img src="assets/verified_bottom_profile_tooltip_live.png" width="450" /> |
...
- **35種類以上の厳選スキルと全カテゴリ**: アーキテクチャ、UI設計、レビュー、オフィス文書、Geminiエコシステムに加え、動的適応「📦 その他」カテゴリを新設。
- **2重の所属識別バッジ**: 各カードに「ネイティブ (青)」と「ユーザー追加 (緑)」バッジを明示し、出自を瞬時に判別。
- **GitHub スキルワンクリック導入**: 最下部に「➕ GitHub からスキルを追加」ボタンを新設。リポジトリURLを渡すだけでAIがインストールセッションを自動開始。
- **ワンクリック呼び出し**: カードをクリックするだけで `$skill-name` を対話欄に自動入力し、カーソルを即座にフォーカス。

| 視覚的スキルハブ (35種類以上の厳選エコシステムスキルを即座に検索) |
| :---: |
| <img src="assets/verified_skills_hub_live.png" width="750" /> |

### サイドバープラグインエコシステムセンター
- **サイドバーネイティブ統合**: 左側ナビゲーションバーに「@ プラグイン」を常駐配置。
- **Codex & Antigravity デュアル互換**: ローカルの OpenAI Codex プラグイン群と Antigravity 拡張機能を自動検出・統括管理。
- **即時呼び出しと柔軟なフィルタ**: すべて、インストール済み、Codex ローカル、オープンソース推薦の4区分でフィルタリング。ディレクトリ展開や対話欄への直接呼び出しに対応。

| サイドバープラグインセンター (9種類の主要プラグイン、分類フィルタと対話欄呼び出し) |
| :---: |
| <img src="assets/verified_plugin_center_live.png" width="750" /> |
